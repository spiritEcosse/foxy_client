...
const AddressForm = () => {
  const { address, setAddress, setAddressAndStore, updateAddressField } =
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    updateAddressField(
      event.target.name as keyof AddressType,
      event.target.value,
    );
  };
  ...
  return (
    <div>
      <Autocomplete
        ...
        fullWidth
        value={
          countries.find((country) => country.id === address?.country_id) ||
          null
        }
        renderInput={(params) => (
          <TextField {...params} required label="Country" variant="outlined" />
        )}
